...
{
    if (checkBoardForWinner(mmboard, 1))
        return false;
    if (checkBoardForWinner(mmboard, -1))
        return false;
    for (int i = 0; i < 9; i++)
...
{
    if (macro_square == other.macro_square && micro_square == other.micro_square)
        return true;
    return false;
}
...
State::State(const State &other) : turn(other.turn), game_over(other.game_over), winner(other.winner), p_last_move(new Move(*other.p_last_move))
{
    for (int i = 0; i < 9; i++)
    {
        for (int j = 0; j < 9; j++)
...
void State::executeMove(Move *move)
{
    ...
    turn = getNextTurn();
    if (p_last_move != nullptr)
    {
        delete p_last_move;
    }
    p_last_move = new Move(*move);
};
